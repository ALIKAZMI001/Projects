...
# ------------------- 🔁 Step 3: Translate English to Urdu -------------------
def translate_to_urdu(english_text):
    translator = Translator()
    try:
        translated = translator.translate(english_text, src='en', dest='ur')
        print("🌍 Urdu Translation:", translated.text)
        return translated.text
    except Exception as e:
        print(f"❌ Urdu Translation Error: {e}")
        return english_text

# ------------------- 🧠 Step 4: Ask GPT via OpenRouter (Free Model) -------------------
def ask_gpt(question):
    ...
    try:
        ...
        print("🧠 GPT Answer (English):", answer)
        return answer
    except Exception as e:
        print("❌ GPT Error:", e)
        return "I'm sorry, I couldn't get the answer right now."

# ------------------- 🔊 Step 5: Speak Urdu Text -------------------
...
def run_voice_assistant():
    ...
    if urdu_text:
        ...
        urdu_reply = translate_to_urdu(gpt_response)
        speak_text(urdu_reply, lang='ur')
    else:
